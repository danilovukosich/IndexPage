...
//AGRREGAR USUARIO BD
app.post('/addUserDB', upload.single(), (request, response) => {
    ...
    request.getConnection((err, conn) => {
        if (err) {
            ...
            conn.query(emailExistsQuery, [obj.email], (queryErr, results) => {
                if (queryErr) {
                    obj_rta.exito = false;
                ...
                if (results.length > 0) {
                    ...
                    conn.query("INSERT INTO users SET ?", [obj], (insertErr, insertResults) => {
                        if (insertErr) {
                            obj_rta.exito = false;
...
app.get('/usersList', (request, response) => {
    request.getConnection((err, conn) => {
        if (err) {
            ...
            conn.query("select id, userName, email, password, profilePicture, profileLevel from users", (err, rows) => {
                if (err)
